						...
						{{-- Cantidad de resultados de la búsqueda --}}
                            @if ($search ?? '')
                                <br><h3><div class="card-title" role="alert">
                                    Hay {{ $subcategory->total() }} resultados para tu búsqueda de '{{ $search }}'.
                                </div></h3>
                            @endif
                            @if ($error ?? '')
							...
							@foreach ($subcategory as $subcategory)
								<tr>
									...
									<td class="admin-button">
										...
										<a href="{{ action('SubcategoryController@edit', $subcategory->id) }}" type="button" class="btn btn-secondary">
